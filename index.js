...
async function sendJoinPrompt(ctx, notJoined) {
  const list = notJoined.map((g) => `• ${g.name} → ${g.url}`).join('\n');
  const text =
    `⚠️ Vào Nhóm Để Được Sử dụng Bot\n\n` +
    `Bạn chưa tham gia đủ nhóm. Vui lòng tham gia các nhóm sau rồi bấm "✅ Tôi Đã Tham Gia":\n\n${list}`;
  ...
  if (notJoined.length > 0) {
    ...
    try {
      await ctx.editMessageText(
        `⚠️ Bạn chưa tham gia đủ, hãy tham gia nhóm còn thiếu:\n\n${list}`,
        joinKeyboard()
      );
    } catch {}
    return;
